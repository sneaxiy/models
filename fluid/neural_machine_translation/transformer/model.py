...
def multi_head_attention(queries,
    ...
    def __compute_qkv(queries, keys, values, n_head, d_key, d_value):
        ...
        """
        #enable_fc_combine = True
        enable_fc_combine = False
        if enable_fc_combine and queries is keys and keys is values:
            q_size = d_key * n_head
            ...
            q = layers.slice(input=qkv, axes=[2], starts=[0], ends=[q_size])
            k = layers.slice(
                input=qkv, axes=[2], starts=[q_size], ends=[2 * q_size])
            v = layers.slice(
                input=qkv, axes=[2], starts=[2 * q_size], ends=[3 * q_size])

            #q, k, v = layers.split(input=qkv, num_or_sections=3)
        elif enable_fc_combine and keys is values:
            q_size = d_key * n_head
            q = layers.fc(input=queries,
                          size=q_size,
                          bias_attr=False,
                          num_flatten_dims=2)
            kv = layers.fc(input=keys,
                           size=2 * q_size,
                           bias_attr=False,
                           num_flatten_dims=2)

            k = layers.slice(input=kv, axes=[2], starts=[0], ends=[q_size])
            v = layers.slice(
                input=kv, axes=[2], starts=[q_size], ends=[2 * q_size])

            #k, v = layers.split(input=kv, num_or_sections=2)
        else:
            q = layers.fc(input=queries,
                          size=d_key * n_head,
                          bias_attr=False,
...
def transformer(src_vocab_size,
    ...
    '''
    softmax = layers.softmax(input=predict, use_cudnn=True)
    cost = layers.cross_entropy(input=softmax,
        label=label, soft_label=True if label_smooth_eps else False)
    '''
